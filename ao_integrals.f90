...
contains
  ...
  subroutine compute_eri_OS(shellA, shellB, shellC, shellD, ERI)
    ...
    if (.not. allocated(boys_table) .or. mmax > boys_mmax) then
      call init_boys_cache(mmax, boys_default_nT, boys_default_Tmin, boys_default_Tmax)
    end if
  ...
  subroutine batch_dgemm(batch, ERI)
    ! Batch processing using BLAS dgemm: computes ERI for a batch of quartets
    type(quartet_t), intent(in) :: batch(:)
    real(dp), intent(inout)     :: ERI(:,:,:,:)
    integer                     :: nbatch, la, lb, lc, ld
    integer                     :: maxAB, maxCD, nrho
    integer                     :: b, i, j, k, l
    real(dp), allocatable       :: Ebra(:,:), R(:,:), Eket(:,:), Work(:,:)

    nbatch = size(batch)
    ! Determine angular momentum sums from first quartet
    la     = basis%shells(batch(1)%i)%l
    lb     = basis%shells(batch(1)%j)%l
    lc     = basis%shells(batch(1)%k)%l
    ld     = basis%shells(batch(1)%l)%l
    maxAB  = la + lb
    maxCD  = lc + ld
    nrho   = maxAB + maxCD + 1

    ! Allocate matrices: [nbatch x nrho], [nrho x nrho], [nrho x nbatch], [nbatch x nrho]
    allocate(Ebra(nbatch,    nrho))
    allocate(R(nrho,         nrho))
    allocate(Eket(nrho,      nbatch))
    allocate(Work(nbatch,    nrho))

    ! Build Ebra, R, Eket for the batch
    do b = 1, nbatch
      i = batch(b)%i; j = batch(b)%j
      k = batch(b)%k; l = batch(b)%l
      ! Compute Hermite intermediates for (i,j) -> fill Ebra(b, 1:nrho)
      ! Compute Hermite intermediates for (k,l) -> fill Eket(1:nrho, b)
      ! Compute Boys factors into R(1:nrho, 1:nrho), based on shared T-values
      ! (Use compute_OS_1D, get_boys, and Prefactor arithmetic.)
    end do

    ! First matrix multiply: Work = Ebra * R
    call dgemm('N','N', nbatch, nrho, nrho, 1.0_dp, Ebra, nbatch, R, nrho, 0.0_dp, Work, nbatch)
    ! Second matrix multiply: Work = Work * Eket
    call dgemm('N','N', nbatch, nbatch, nrho, 1.0_dp, Work, nbatch, Eket, nrho, 0.0_dp, Work, nbatch)

    ! Scatter diagonal elements back into global ERI array
    do b = 1, nbatch
      i = batch(b)%i; j = batch(b)%j
      k = batch(b)%k; l = batch(b)%l
      ERI(i,j,k,l) = Work(b,b)
    end do

    ! Clean up
    deallocate(Ebra, R, Eket, Work)
  end subroutine batch_dgemm
